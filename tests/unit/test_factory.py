...
class TestSQLMeshAssetsFactory:
    ...
    def test_sqlmesh_assets_factory_with_op_tags(
        self, sqlmesh_resource: SQLMeshResource
    ) -> None:
        ...
        assets = sqlmesh_assets_factory(
            sqlmesh_resource=sqlmesh_resource, op_tags=op_tags
        )

        # Check that assets are created (op tags are not directly accessible)
        assert len(assets) > 0

    def test_sqlmesh_assets_factory_with_owners(
        self, sqlmesh_resource: SQLMeshResource
    ) -> None:
        ...
        assets = sqlmesh_assets_factory(
            sqlmesh_resource=sqlmesh_resource, owners=owners
        )
...
class TestSQLMeshAdaptiveScheduleFactory:
    ...
    def test_sqlmesh_adaptive_schedule_factory_basic(
        self, sqlmesh_resource: SQLMeshResource
    ) -> None:
    ...
    def test_sqlmesh_adaptive_schedule_factory_with_custom_name(
        self, sqlmesh_resource: SQLMeshResource
    ) -> None:
        """Test adaptive schedule factory with custom name."""
        custom_name = "my_custom_schedule"
        schedule, job, assets = sqlmesh_adaptive_schedule_factory(
            sqlmesh_resource=sqlmesh_resource, name=custom_name
        )

        assert schedule.name == custom_name

    def test_sqlmesh_adaptive_schedule_factory_schedule_properties(
        self, sqlmesh_resource: SQLMeshResource
    ) -> None:
...
class TestFactoryIntegration:
    ...
    def test_factory_integration_with_materialization(
        self, sqlmesh_resource: SQLMeshResource
    ) -> None:
