...
class TestBlockingParameter:
    ...
    def test_blocking_parameter_default(self, sqlmesh_resource: SQLMeshResource) -> None:
        """Test that the blocking parameter is correctly set to True by default."""
    ...
    def test_blocking_parameter_with_custom_audit(self, sqlmesh_resource: SQLMeshResource) -> None:
        ...
        for snapshot in models:
            if hasattr(snapshot, 'node') and hasattr(snapshot.node, 'audits') and snapshot.node.audits:
        ...
        if model_with_audits:
            ...
            # Create asset checks from the model
            asset_key = AssetKey(["jaffle_test", "sqlmesh_jaffle_platform", model_with_audits.name.split('.')[-1]])
