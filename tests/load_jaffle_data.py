...
def load_csv_to_duckdb(
    csv_path: str,
    table_name: str,
    db_path: str = "tests/fixtures/sqlmesh_project/jaffle_test.db",
):
    """
    ...
    df = pd.read_csv(csv_path)  # noqa: F841 - Used in DuckDB SQL query below
...
def main():
    ...
    csv_files_exist = all(
        os.path.exists(csv_path) for csv_path in csv_to_table_mapping.keys()
    )
