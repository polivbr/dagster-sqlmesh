...
def generate_products():
    ...
    products = [
        ...
        ["JAF-003", "the krautback", "food", 1200, "lamb and pork bratwurst with house-pickled cabbage sauerkraut and mustard"],
...
def generate_stores():
    ...
    """
    stores = [
        ["10fa6637-5363-44dd-8202-1fca446cd778", "Philadelphia", "2018-09-01T00:00:00", 0.06],
        ["220b1ab8-3567-4ccb-ba36-e210d9fe6884", "Brooklyn", "2019-03-12T00:00:00", 0.04],
        ["0f8baf84-89b7-4bfd-88ec-47bc168fed09", "Chicago", "2020-04-28T00:00:00", 0.0625],
        ["a1b2c3d4-e5f6-7890-abcd-ef1234567890", "Los Angeles", "2021-01-15T00:00:00", 0.085],
        ["b2c3d4e5-f6g7-8901-bcde-f23456789012", "Miami", "2021-06-20T00:00:00", 0.07],
        ["c3d4e5f6-g7h8-9012-cdef-345678901234", "Seattle", "2021-11-10T00:00:00", 0.065],
    ]
    return stores
...
def generate_tweets(customers, num_tweets=500):
    ...
    products = ["adele-ade", "tangaroo", "mel-bun", "flame impala", "doctor stew", "krautback"]
    ...
    for i in range(num_tweets):
        ...
        tweets.append([
            tweet_id, user_id, tweet_date.isoformat(), content
        ])
...
def main():
    ...
    write_csv(products, "raw_source_products.csv", ["sku", "name", "type", "price", "description"])
    write_csv(stores, "raw_source_stores.csv", ["id", "name", "opened_at", "tax_rate"])
    write_csv(supplies, "raw_source_supplies.csv", ["id", "sku", "name", "cost", "perishable"])
    write_csv(orders, "raw_source_orders.csv", ["id", "customer", "ordered_at", "store_id", "subtotal", "tax_paid", "order_total"])
    write_csv(items, "raw_source_items.csv", ["id", "order_id", "sku"])
    write_csv(tweets, "raw_source_tweets.csv", ["id", "user_id", "tweeted_at", "content"])
